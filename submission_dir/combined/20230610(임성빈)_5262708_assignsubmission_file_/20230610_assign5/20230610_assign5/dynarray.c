...
/* Belows functions are Added functions by myself */

// Convert token array to a character array and handle input/output redirection
void DynArray_toCharArray(DynArray_T oDynArray, char **CharArray, char **redirect) {
    assert(oDynArray != NULL);
    assert(DynArray_isValid(oDynArray));
    assert(CharArray != NULL);

    int arrayIndex = 0;
    char *redirectionIn = NULL;
    char *redirectionOut = NULL;

    for (int idx = 0; idx < DynArray_getLength(oDynArray); ++idx) {
        struct Token *currentToken = (struct Token *)DynArray_get(oDynArray, idx);

        if (currentToken->eType == TOKEN_REDIN) {
            if (idx + 1 < DynArray_getLength(oDynArray)) {
                struct Token *nextToken = (struct Token *)DynArray_get(oDynArray, idx + 1);
                redirectionIn = nextToken->pcValue;
            }
            idx++; // Skip the next token since it's processed as redirection
        }
        else if (currentToken->eType == TOKEN_REDOUT) {
            if (idx + 1 < DynArray_getLength(oDynArray)) {
                struct Token *nextToken = (struct Token *)DynArray_get(oDynArray, idx + 1);
                redirectionOut = nextToken->pcValue;
            }
            idx++; // Skip the next token since it's processed as redirection
        }
        else {
            CharArray[arrayIndex++] = currentToken->pcValue;
        }
    }

    redirect[0] = redirectionIn;
    redirect[1] = redirectionOut;
    CharArray[arrayIndex] = NULL;
}

// Remove elements from the dynamic array within the specified range
void DynArray_removeRange(DynArray_T tokens, int start, int end) {
    assert(tokens != NULL);
    assert(start >= 0 && start < DynArray_getLength(tokens));
    assert(end > start && end <= DynArray_getLength(tokens));

    int count = end - start;
    for (int i = 0; i < count; ++i) {
        struct Token *tokenToRemove = (struct Token *)DynArray_get(tokens, start);
        free(tokenToRemove);
        DynArray_removeAt(tokens, start);
    }
}
