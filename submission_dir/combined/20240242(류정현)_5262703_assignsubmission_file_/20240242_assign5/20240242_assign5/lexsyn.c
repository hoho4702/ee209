...
createToken(DynArray_T oTokens, enum TokenType ttype, char *value)
{
  ...
  /* Print the created token */
  // if (value != NULL) {
  //   printf("Created Token: Type = %d, Value = '%s'\n", ttype, value);
  // } else {
  //   printf("Created Token: Type = %d, Value = NULL\n", ttype);
  // }
...
void
command_lexLine(const char *pcLine, DynArray_T cTokens)
{
...
lexLine(const char *pcLine, DynArray_T oTokens)
{
  ...
  for (;;) {
    if (iLineIndex == MAX_LINE_SIZE) {return LEX_LONG;}
    /* "Read" the next character from pcLine. */
    c = pcLine[iLineIndex++];
    ...
    switch (eState) {
      ...
      case STATE_START:
        if ((c == '\n') || (c == '\0')) {return LEX_SUCCESS;}
        else if (isspace(c)) {eState = STATE_START;}
        else if (c == '|') {
          /* Create a PIPE token. */
          if (createToken(oTokens, TOKEN_PIPE, NULL) == FALSE) {return LEX_NOMEM;}
          eState = STATE_START;
        }
        else if (c == '&') {
          // Create a Background command token.
          if (createToken(oTokens, TOKEN_BG, NULL) == FALSE) {return LEX_NOMEM;}
          eState = STATE_START;
        }
        else if (c == '>') {
          /* Create a REDOUT token. */
          if (createToken(oTokens, TOKEN_REDOUT, NULL) == FALSE) {return LEX_NOMEM;}
          eState = STATE_START;
        }
        else if (c == '<') {
          /* Create a PIPE token. */
          if (createToken(oTokens, TOKEN_REDIN, NULL) == FALSE) {return LEX_NOMEM;}
          eState = STATE_START;
        }
        else if (c == '\"') {eState = STATE_IN_DQUOTE;}
        else if (c == '\'') {eState = STATE_IN_QUOTE;}
      ...
      case STATE_IN_WORD:
        if ((c == '\n') || (c == '\0')) {
          /* Create a WORD token. */
          acValue[iValueIndex] = '\0';
          if (createToken(oTokens, TOKEN_WORD, acValue) == FALSE) {return LEX_NOMEM;}
          iValueIndex = 0;
          return LEX_SUCCESS;
        }
        else if (isspace(c)) {
