/* 20230540 Lee Jaeyun */
/* This file implements a dynamic array data structure 
that can automatically resize itself as elements are added or 
removed. It provides functions for creating, freeing, and       
manipulating the dynamic array, including adding, removing,      
accessing, and sorting elements. It also includes linear and     
binary search functionalities for searching elements. The     
dynamic array is implemented using an array of void pointers, 
...
/*--------------------------------------------------------------------*/
/* Parse tokens from a dynamic array into a  character array and redirection array. */

void DynArray_parseTokens(DynArray_T oDynArr, char **CharArray, char **redirection) {
  assert(oDynArr != NULL);
  assert(DynArray_isValid(oDynArr));
  assert(CharArray != NULL);

  int idx = 0;
  enum {NONE, REDIN, REDOUT} state = NONE;

  for (int i = 0; i < DynArray_getLength(oDynArr); ++i) {
    struct Token *token = (struct Token *)DynArray_get(oDynArr, i);
    ...
    default:
      if (state == REDIN) {
        redirection[0] = token->pcValue;
        state = NONE;
      } else if(state == REDOUT) {
        redirection[1] = token->pcValue;
        state = NONE;        
      } else {
        CharArray[idx++] = token->pcValue;
      }
    } 
  }
  CharArray[idx] = NULL;
...
/*--------------------------------------------------------------------*/
/* Remove a range of tokens from a dynamic array. */

void DynArray_removeRange(DynArray_T oTokens, int startidx, int endidx) {
  assert(oTokens != NULL);
  assert(startidx >= 0 && startidx < DynArray_getLength(oTokens));
  assert(endidx > startidx && endidx <= DynArray_getLength(oTokens));

  for (int i = startidx; i < endidx; i++) {
    struct Token *t = DynArray_get(oTokens, startidx);
    free(t);
    DynArray_removeAt(oTokens, startidx);
  }
}