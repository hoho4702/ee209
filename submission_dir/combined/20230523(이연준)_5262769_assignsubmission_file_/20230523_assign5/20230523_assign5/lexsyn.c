/*--------------------------------------------------------------------*/
/* lexsyb.c - 20230523 Yeonjun Lee                                    */
/* This source code define some functions                             */
/*  related to analyze given comman                                   */
/*--------------------------------------------------------------------*/
...
FIXME:The reason why we use duplicated-like function is simple 'lexLine'
lesical analysis guide-line in EE209
shows ' as a normal character, not quotes.
So, we need to handle this as a normal character
  ...
  for (i = 0; i < DynArray_getLength(oTokens); i++) {
    ...
    if (i == 0) {
      ...
      if (t->eType == TOKEN_PIPE) {
        /* No redout in previous tokens and no
          consecutive pipe in following tokens */
        if (roexist == TRUE) {
