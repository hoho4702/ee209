...
#ifndef NDEBUG           //이게 정의되어있지 않을 때에만 코드 실행
...

{
  ...
  oDynArray->ppvArray = (const void**)calloc((size_t)oDynArray->iPhysLength,sizeof(void*));
...

{
  if (oDynArray == NULL)
    return;
  for(int index=0; index<DynArray_getLength(oDynArray);index++){
    if((oDynArray->ppvArray)[index]!=NULL) {
      struct Token* t= (struct Token*) oDynArray->ppvArray[index];
      freeToken(t,NULL);
    }
  }
...
void convertDynArrayToStrings(DynArray_T dynArr, char **outStrings, char **redirTargets) {
  assert(dynArr != NULL);
  assert(DynArray_isValid(dynArr));
  assert(outStrings != NULL);

  int isInput = 0, isOutput = 0;
  int outIndex = 0;
  int arrSize = DynArray_getLength(dynArr);

  for (int idx = 0; idx < arrSize; idx++) {
    struct Token *pToken = (struct Token *)DynArray_get(dynArr, idx);
    ...
    if (isInput) {
      redirTargets[0] = pToken->pcValue;
      isInput = 0;
      } 
    else if (isOutput) {
      redirTargets[1] = pToken->pcValue;
      isOutput = 0;
    } 
    else {
      outStrings[outIndex++] = pToken->pcValue;
    }
  }

  outStrings[outIndex] = NULL;
}
