...
checkBuiltin(struct Token *t) {
  ...
  if (strncmp(t->pcValue, "cd", 2) == 0 && strlen(t->pcValue) == 2)
    return B_CD;
  else if (strncmp(t->pcValue, "exit", 4) == 0 && strlen(t->pcValue) == 4)
    return B_EXIT;
...
// return True(1) if token type is word(TOKEN_WORD)
int checkWordToken(struct Token *t) {
  assert(t);
  return t->eType == TOKEN_WORD;
}

// convert token value to string
const char* tokenToStr(struct Token *t) {
  assert(t);
  assert(t->pcValue);
  return (const char*)t->pcValue;
}
