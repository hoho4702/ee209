...
int main(void){
    ...
    while((ipch=getchar())!=EOF){ //표준입력을 받아 개수 세는 함수
        ...
        if(li){//주석 내부가 아닐 때만 문자 수 새기
            ...
            if((isspace(pre_ch)&&!isspace(ipch))&&ipch!='/'&&blc){ 
                words++;
            }
        ...
        /*
        if(ipch == '\n'){//for debugging.
            printf("character : /n, LF : %d words : %d char : %d\n", lines, words, chars);
        }
        
        else{
            printf("character : %c, LF : %d words : %d char : %d\n", ipch, lines, words, chars);
        }
        */
