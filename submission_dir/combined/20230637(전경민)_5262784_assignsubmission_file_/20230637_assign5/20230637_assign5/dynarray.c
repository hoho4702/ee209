...
// Convert array of tokens to a character array and handle i/o redirection
void DynArray_tocharArray(DynArray_T oDynArray, char **CharArray, char **ioRedirect) {
  assert(oDynArray != NULL);
  assert(DynArray_isValid(oDynArray));
  assert(CharArray != NULL);
    
  int index = 0;
  int redirectState = 0; // 0: NONE, 1: INPUT_REDIRECT, 2: OUTPUT_REDIRECT

  int i;
  for(i = 0; i < DynArray_getLength(oDynArray); ++i){
    struct Token *tok = (struct Token *)DynArray_get(oDynArray, i);


    if (tok->eType == TOKEN_REDIN)  redirectState = 1; // INPUT_REDIRECT
    else if (tok->eType == TOKEN_REDOUT)  redirectState = 2; // OUTPUT_REDIRECT
    else {
      if (redirectState == 1) { // INPUT_REDIRECT
        ioRedirect[0] = tok->pcValue;
        redirectState = 0; // NONE
      }
      else if (redirectState == 2) { // OUTPUT_REDIRECT
        ioRedirect[1] = tok->pcValue;
        redirectState = 0; // NONE
      }
      else {
        CharArray[index] = tok->pcValue;
        index++;
      }
    }
  }
...
// Remove dynarray elements in range
void DynArray_rmvElements (DynArray_T toks, int startIndex, int endIndex) {
  assert(toks != NULL);
  assert(startIndex >= 0 && startIndex < DynArray_getLength(toks));
  assert(endIndex > startIndex && endIndex <= DynArray_getLength(toks));

  int i;
  for(i = startIndex; i < endIndex; ++i){
    struct Token *tok = DynArray_get(toks, startIndex);

    free(tok);
    DynArray_removeAt(toks, startIndex);
  }
}
