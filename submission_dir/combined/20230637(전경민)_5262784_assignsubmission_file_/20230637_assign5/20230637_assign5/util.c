...
void execute_CD(DynArray_T oTokens) {
  ...
  if (argc == 1) {
    ...
    if (chdir(home_dir) == -1) errorPrint(NULL, PERROR);
  }
  else if (argc > 2)  errorPrint("cd takes one parameter", FPRINTF);
  else {
...
void execute_SETENV(DynArray_T oTokens) {
  ...
  else if (argc<2 || argc>3)  errorPrint("setenv takes one or two parameter", FPRINTF);
  else {
    const char *dir = ((struct Token *)DynArray_get(oTokens, 1))->pcValue;
